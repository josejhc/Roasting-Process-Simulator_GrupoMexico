...
encoding_dim = 20  # Ajusta según cuántas variables quieras conservar
...
# Calcular error de reconstrucción por variable
reconstruction_error = np.mean((X_scaled - X_reconstructed) ** 2, axis=0)
error_por_variable = pd.Series(reconstruction_error, index=clean_data.columns)
error_ordenado = error_por_variable.sort_values(ascending=False)

# Imprimir todas las variables ordenadas por importancia
print("Variables ordenadas por error de reconstrucción (mayor error = mayor importancia):")
for var, err in error_ordenado.items():
    print(f"{var}: {err:.6f}")
